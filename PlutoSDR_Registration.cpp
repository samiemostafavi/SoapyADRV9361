...
static std::vector<SoapySDR::Kwargs> find_PlutoSDR(const SoapySDR::Kwargs &args){
	...
	if(ret == 0){
		...
		if (args.count("hostname") == 0) {
			//return results; // schiessl
			// schiessl: instead of returning, specify the default hostname
			//std::string myhostname = "10.0.9.1"; // schiessl: default hostname
			//ctx = iio_create_network_context(myhostname.c_str()); // schiessl
			char myhostname[16] = "10.0.9.1"; // schiessl: default hostname
			ctx = iio_create_network_context(myhostname); // schiessl
			options["hostname"] = myhostname; // schiessl
		} else { // schiessl
		...
		if(ctx == nullptr) {
			SoapySDR_logf(SOAPY_SDR_ERROR, "host not found\n"); // schiessl
